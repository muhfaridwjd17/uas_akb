...
async function renderJadwalPublik() {
  ...
  // ===== SECTION STATUS SEMUA RUANGAN =====
  const semuaRuanganList = STATE.data.ruangan.map(r => r['Nama Ruangan']).filter(Boolean).sort();
  const ruanganFinal = semuaRuanganList.length > 0
    ? semuaRuanganList
    : [...new Set(jadwal.map(j => j.Ruangan).filter(Boolean))].sort();

  if (ruanganFinal.length > 0) {
    html += `
      <div style="margin-top:40px;padding-top:24px;border-top:2px solid var(--border);">
        <div style="font-weight:800;font-size:16px;margin-bottom:16px;color:var(--text-primary);">
          &#x1F4CD; Status Semua Ruangan
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:8px;">
          ${ruanganFinal.map(namaR => {
            const sR    = STATUS_KULIAH_DATA[namaR] || {};
            const sNow  = sR.status || '';
            const isS   = sNow === 'Sedang Dipakai';
            const isB   = sNow === 'Dibooking';
            const bg    = isS ? '#052e16' : isB ? '#0f172a' : 'var(--bg-elevated)';
            const bdr   = isS ? '#16a34a' : isB ? '#2563eb' : 'var(--border)';
            const clr   = isS ? '#4ade80' : isB ? '#60a5fa' : 'var(--text-muted)';
            const icon  = isS ? '&#x1F7E2;' : isB ? '&#x1F4C5;' : '&#x2B1C;';
            const label = isS ? 'Sedang Dipakai' : isB ? 'Dibooking' : 'Kosong';
            const sub   = isS || isB
              ? `<div style="font-size:10px;color:${clr};opacity:.8;margin-top:2px;">${sR.mataKuliah||''}</div>
...
function drawStatusKuliah() {
  ...
  let html = `
    ...
    ${(() => {
      ...
      // Hanya tampilkan notifikasi TERBARU saja (1 item, bukan lama-lama)
      const keluarFiltered = keluar.filter(p => (p.Status === 'Disetujui' || p.Status === 'Ditolak') && !_dismissed.includes(p.ID));
      const keluarAktif = keluarFiltered.length > 0 ? [keluarFiltered[keluarFiltered.length - 1]] : [];
    ...
    <div class="rm-grid">
      ${semuaRuangan.map(rObj => {
        ...
        // Cek apakah ada permintaan booking yang masih Menunggu untuk ruangan ini
        const adaPermintaanMenunggu = (PERMINTAAN_DATA.keluar || []).some(p =>
          p['Ruangan'] === namaR && p.Status === 'Menunggu'
        );
        ...
        } else if (adaPermintaanMenunggu) {
          state = 'dibooking'; tagText = 'Menunggu ACC'; pillText = 'Menunggu';
        } else if (adaJadwal) {
        ...
        if (isLocked) {
          ...
          // Permintaan sedang menunggu ACC — bisa dibatalkan
          const permintaan = (PERMINTAAN_DATA.keluar || []).find(p => p['Ruangan'] === namaR && p.Status === 'Menunggu');
          btnHTML = `<button class="abtn stop" onclick="batalPermintaan('${permintaan?.ID || ''}','${namaR}')">✕ Batalkan</button>`;
        } else if (adaJadwal) {
...
function batalPermintaan(id, namaR) {
  if (!id) { showToast('⚠️ ID permintaan tidak ditemukan', 'warning'); return; }
  if (!confirm('Batalkan permintaan booking untuk ' + namaR + '?')) return;

  // Hapus dari data lokal
  const idx = (PERMINTAAN_DATA.keluar || []).findIndex(p => p.ID === id);
  if (idx > -1) PERMINTAAN_DATA.keluar.splice(idx, 1);

  // Simpan ke dismissed supaya tidak muncul lagi
  try {
  ...
  apiPost('tolakPermintaan', { id, alasan: 'Dibatalkan oleh peminta', namaKetua: KETUA_SESSION?.nama || '' });
  showToast(`✕ Permintaan booking ${namaR} dibatalkan`, 'info');
  drawStatusKuliah();
}
